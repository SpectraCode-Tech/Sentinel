...
export default function CommentSection({ articleId }) {
        ...
        toast((t) => (
            <div className="flex flex-col gap-3 p-1 min-w-[250px]">
                ...
                <div className="flex justify-end gap-4 border-t border-slate-100 pt-3 mt-1">
                    ...
                    <button
                        onClick={() => { toast.dismiss(t.id); executeDelete(commentId); }}
            ...
            style: {
                ...
                padding: '16px',
                color: '#0f172a',
                borderRadius: '4px', // Matches the professional/serious aesthetic
    ...
    const executeDelete = async (id) => {
        ...
        toast.promise(
            deletePromise,
            {
                loading: 'Removing dispatch...',
                success: 'Entry deleted', // This will now disappear after 2s
                error: 'Could not remove entry',
            },
            {
                position: 'top-center',
                // --- THE FIX: Force the success/error toasts to vanish ---
                success: {
                    duration: 2000,
                },
                error: {
                ...
                style: {
                    ...
                    borderRadius: '0px', // Matches your "Serious" UI
                    border: '1px solid #e2e8f0'
                },
            }
        );
                    ...
                    {token && (
                        <div className="flex justify-end pb-3">
                            <button
                                ...
                                className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-900 hover:text-blue-600 disabled:text-slate-300 transition-colors flex items-center gap-2"
                            >
                    ...
                    <div key={comment.id} className="group">
                        <div className="flex flex-col">
                            ...
                            <div className="pl-9">
                                ...
                                {/* Nested Replies - Clean Linear Style */}
                                {comment.replies?.length > 0 && (
                                    <div className="space-y-8 border-l border-slate-100 pl-8 ml-1">
                                        {comment.replies.map((reply) => (
                                            <div key={reply.id}>
                                                <header className="flex items-center gap-2 mb-2">
                                                    <span className="text-[11px] font-bold text-slate-900 uppercase tracking-tight">{reply.user}</span>
                                                    <span className="text-[9px] text-slate-300 uppercase tracking-tighter">{new Date(reply.created_at).toLocaleDateString()}</span>
                                                </header>
                                                <p className="text-sm leading-relaxed text-slate-600 mb-3 italic">
                                                    {reply.content}
                                                </p>
                                                <div className="flex items-center gap-4">
                                                    <button onClick={() => handleReplyClick(reply)} className="text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:text-slate-900 transition-all underline decoration-transparent hover:decoration-slate-200">Reply</button>
                                                    {(user?.username === reply.user || user?.role === "ADMIN") && (
                                                        <button onClick={() => confirmDelete(reply.id)} className="text-[9px] font-bold text-slate-200 hover:text-rose-500 transition-colors uppercase tracking-widest">Delete</button>
                                                    )}
                                                </div>
                                            </div>
                                        ))}
